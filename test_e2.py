...
for f in onlyfiles:
    p = subprocess.Popen("cat " + tests_path + f + " | ./etapa2", stdout=subprocess.PIPE, shell=True)
    message = p.communicate()[0] 
    if (message and (f not in incorrect)):
        print(f + " should be correct - " + message)
    if (not message and f in incorrect):
        print(f + " should be incorrect - " + message)
